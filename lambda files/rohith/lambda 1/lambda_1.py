...
#def sns_notification(error_type, message):
    # Send SNS notification on error or condition
    #response = sns.publish(
        #TopicArn='arn:aws:sns:us-east-2:392523246457:Failure_notification',
        #Message=message,
    #)
...
def crawler_trigger(crawler_name):
    ...
    try:
        ...
        if status == 'READY':
            ...
            print(f"Crawler '{crawler_name}' has started.")
        else:
            print(f"Crawler '{crawler_name}' is already running. Status: {status}")
...
def athena_query_call(Query):
    ...
    # Fetch data
    #Let us consider Bloomberg = input_key_1
    #Let us consider Bondedge = input_key_2
    while True:
            ...
            for obj in page['Contents']:
                ...
                if '.csv' in input_key or '.txt' in input_key:
                    ...
                    if 'bondedge' in input_key:
                        ...
                        wr.s3.to_parquet(df, path=f"s3://{output_bucket}/amica-a039986-prahalad-investments/output_test_bucket/test-lambda-investments/investments/inv_bondedge_test/{filename}.snappy.parquet", index=False, compression="snappy")
                        print(f"s3://{output_bucket}/amica-a039986-prahalad-investments/output_test_bucket/test-lambda-investments/investments/inv_bondedge_test/{filename}.snappy.parquet has been saved.")
                    ...
                    # Bloomberg process
                        ...
                        try:
                            df = pd.read_csv(data, sep = ',', names= col_names, dtype = {'amica_portfolio_id':'str','amica_portfolio_desc':'str','pricing_dt':'str','cusip_id':'str','security_nm':'str','security_typ':'str','holdings_qty':'float64','price_close_amt':'float64','currency_typ':'str'}, header = None, skiprows = 1, skipfooter=1, skipinitialspace=True, engine='python')
                        except Exception as e:
                        ...
                        if pricing_dt == filename_dt:
                            wr.s3.to_parquet(df, path=f"s3://{output_bucket}/amica-a039986-prahalad-investments/output_test_bucket/test-lambda-investments/investments/inv_bloomberg_test/{filename}.snappy.parquet", index=False, compression="snappy")
                            print(f"s3://{output_bucket}/amica-a039986-prahalad-investments/output_test_bucket/test-lambda-investments/investments/inv_bloomberg_test/{filename}.snappy.parquet has been saved.")
...
def lambda_handler(event, context):
    try:
        ...
        # If event doesn't contain 'Records', use the default test values
        print("No S3 trigger found. Using default test values.")
        input_bucket = 'amica-a035527-useast2-392523246457-dev'
        input_key = 'amica-a039986-prahalad-investments/test-investment/bloomberg/fixed-income/01/'
        #input_key = 'amica-a039986-prahalad-investments/test-investment/bondedge/fixed-income/01/'
    ...
    if 'bondedge' in input_key:
        ...
        response = lambda_client.invoke(
            FunctionName='arn:aws:lambda:us-east-2:392523246457:function:Lambda_test_2',
