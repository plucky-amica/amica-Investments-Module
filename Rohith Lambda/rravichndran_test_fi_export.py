...
def process(current_date):
    ...
    Bond_check_query = f"select case when count(*)>0 then true else false end as check_flag from amica_eds_dev_engineers_test_db.inv_bondedge where pricing_dt = date('{exp_date}');"
    s3_loc_ = "s3://amica-aws-eds-dev-engineers-test/check_query/"
    ...
    if flag_bond == True:
        ...
        Bloom_check_query = f"select case when count(*)>0 then true else false end as check_flag from amica_eds_dev_engineers_test_db.inv_bloomberg where pricing_dt = date('{exp_date}');"
        s3_loc_ = "s3://amica-aws-eds-dev-engineers-test/check_query/"
    ...
    Query2 = f"select * from amica_eds_dev_engineers_test_db.inv_ref_trading_days_net_v where calendar_dt between date('{exp_date}') and date('{current_date}') and asset_class_desc = 'fixed income';"
    ...
    Query3 = f'select * from amica_eds_dev_engineers_test_db.inv_fia_factset_output_v where "pricing date"=\'{exp_date_str}\';'
    ...
    output_filepath = f's3://amica-aws-datalake-general-dev-factset-landing/Outgoing/DailyFSAttr/DailyFSAttr_{current_date.replace('-','')}.csv'
    output_filepath2 = f's3://amica-aws-eds-dev-engineers-test/investments/inv_fia_factset_output_csv/{current_date}/DailyFSAttr_{current_date.replace('-','')}_{filename_ts_add.replace(' ','_')}.csv'
    
    output_logging_filepath = f's3://amica-aws-eds-dev-engineers-test/investments/inv_fia_logging_outputs/DailyFSAttr_{current_date.replace('-','')}_{filename_ts_add}_logging.csv'
